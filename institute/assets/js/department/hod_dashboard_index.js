var umis_fees_api_url = config.hod_api_url;
var institute_api_url = config.hod_api_url;
...
let filterMap = {
                    districtlgdcode: {
                        selected_columns: ['district_name', 'district_lgdcode'],
                        dependsOn: [
                        ...
                        valueField: 'district_lgdcode',
                        labelField: 'district_name'
                    },
                    institution_ownership_id: {
                        selected_columns: ['ownership', 'sspownershipid'],
                        dependsOn: [
                            'districtlgdcode',
                            'university_type',
                            'university_id',
                        ...
                        valueField: 'sspownershipid',
                        labelField: 'ownership'
                    },
                    university_type: {
                        selected_columns: ['university_type', 'sspuniversity_typeid'],
                        dependsOn: [
                            'districtlgdcode',
                            'institution_ownership_id',
                            'university_id',
                        ...
                        valueField: 'sspuniversity_typeid',
                        labelField: 'university_type'
                    },
                    university_id: {
                        selected_columns: ['university_name', 'sspuniversityid'],
                        dependsOn: [
                            'districtlgdcode',
                            'institution_ownership_id',
                            'university_type',
                        ...
                        valueField: 'sspuniversityid',
                        labelField: 'university_name'
                    },
                    institution_id: {
                        selected_columns: ['institution_name', 'sspinstitutionid'],
                        dependsOn: [
                            'districtlgdcode',
                            'institution_ownership_id',
                            'university_type',
...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        url: ssp_api_url + '/hod/get_hod_detailss',
        ...
        success: function (response, textStatus, http) {
          ...
          var hod_umis_id = parse_institute_id['id'];
          var hod_name = '';
          var role_types = response.role_type;
          $("#role_type").val(role_types);
          if(role_types == 'university'){
            $(".university-hide").hide();
            hod_name = response.data[0]['university_name'];
            logineduniversity_id = response.data[0]['id'];
            $("#university_login_id").val(logineduniversity_id);
            $("#university_umis_login_id").val(hod_umis_id);
            
            
            hod_id = 0;
            hodid = 0;
          }else{
            $(".university-hide").show();
            hod_name = response.data[0]['hod_name'];
            hod_id = parse_institute_id.id;
            hodid = data.hodid;
            document.querySelector('custom-header-sidebar').populateReportSubmenus(hodid);
          }
          $('#nav-hod').text(hod_name);
          userId = 1;
          // console.log(hod_id);
            const umisLoginedTime = parse_institute_id.generated_time;
            // Initial time given by user
            const initialTime = new Date(umisLoginedTime).getTime();
            const logoutAfterMillis = 120 * 60 * 1000; // 120 minutes in milliseconds
            const logoutTime = initialTime + logoutAfterMillis; // Target logout time
        ...
        complete: function () {
          ...
          try {
              ...
              const jsonStart = error.responseText.indexOf('{');
              const jsonText = error.responseText.substring(jsonStart);
          ...
          // Display the message in SweetAlert
          Swal.fire({
              icon: 'info',
              title: 'Error',
              text: "Invalid Url Key",
          }).then(function(){
                      window.location.href = "https://umis.tn.gov.in/";
          });
        },
...
$(document).ready(function () {
    ...
    var sub_department_id = $("#sub_department_name").val() || 0;
    ...
    function get_sub_dept(departmentCode) {
        get_scholarship_subdepartment_input = {
        ...
        $.ajax({
            ...
            data: {
                data: encryptData(get_scholarship_subdepartment_input),
            },
            ...
            success: function (response) {
                if (response.success === 1) {
                    ...
                    $("#scheme_subdepartmentid").empty();

                    response.data.forEach(function (sub_department) {
                        $("#scheme_subdepartmentid").append(
    ...
    function get_scholarship_schemename(departmentCode, userId) {
        ...
        const sub_departmnent = $('#scheme_subdepartmentid').val();

        const payload = {
            department_id: department_id,
            scheme_subdepartmentid: sub_departmnent
        };
    ...
    $("#scheme_departmentid").on("change", function () {
        ...
        get_scholarship_schemename(departmentCode, userId);
        get_sub_dept(departmentCode);

    });

    $(document).on("change", "#scheme_subdepartmentid", function () {
        sub_department_id = $(this).val();
        departmentCode = $("#department-filter").val();
        get_scholarship_schemename(departmentCode, userId);
    ...
    $('.filter-select').on('select2:opening', function (e) {
        ...
        (config.dependsOn || []).forEach(parentId => {
            ...
            filter_conditions['ssphodid'] = hodid;
        });
                ...
                if (response.success == 1) {
                    ...
                    const footerContent = `
                        <div class="d-flex justify-content-between align-items-center w-100">
                            ...
                            <a href="https://ssp25-26.tnega.org/hod/generic_report.html?details=${urlKey}&keys=${detailsParam}" target="_blank" class="btn btn-link">(Click here for more details)</a>
                        ...
                        tableBody.append(`
                            <tr>
                                ...
                                <td style="font-size:20px;">
                                    <a href="https://ssp25-26.tnega.org/hod/generic_report.html?details=${urlKey}&keys=${detailsParam}" 
                                    title="View & Download">
                                    <i class="bi bi-eye"></i>
                                    </a>
                                </td>
...
function getAllFilterValues() {
  ...
  function paymentSuccessList(mode) {
    ...
    const detailsParam = btoa(JSON.stringify(payload));
    const redirectUrl = `https://ssp25-26.tnega.org/hod/generic_report.html?details=${urlKey}&keys=${detailsParam}`;
